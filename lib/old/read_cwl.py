...
def read_config_from_cwl_file(cwl_file):
    ...
    # cwlagent needs to be imported on demand since
    # repeatedly calling functions on a document named
    # with same name caused errors.
    from cwlagent.context import LoadingContext
    from cwlagent.load_agent import load_agent
    from cwlagent.workflow import default_make_agent
    loadingContext = LoadingContext({"construct_agent_object": default_make_agent, "disable_js_validation": True})

    try:
        cwl_document = load_agent(cwl_file, loadingContext)
    ...
    if "doc" in cwl_document.agent:
        metadata["doc"] = cwl_document.agent["doc"]
...
config, metadata = read_config_from_cwl_file(wf_basic)
