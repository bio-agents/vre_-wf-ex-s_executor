...
from agent.VRE_Agent import WfExS


class Wrapper:
    """
    Functions for wrapping the agent set up and execution.
    """
    ...
    def __init__(self, configuration=None):
        ...
        which are specific to WfExS agent.
    ...
    def run(self, input_files, input_metadata, output_files, output_metadata):
        """
        Main run function for running WfExs agent.
        ...
        try:
            ...
            errstr = "Agent WfExS wasn't executed successfully. ERROR: {}".format(error)
            logger.error(errstr)
...
def main_wrapper(config_path, in_metadata_path, out_metadata_path):
    ...
    This function launches the agent using configuration written in two json files: config.json and in_metadata.json.

    :param config_path: path to a valid VRE JSON file containing information on how the agent should be executed.
    :type config_path: str
    :param in_metadata_path: path to a valid VRE JSON file containing information on agent inputs.
    :type in_metadata_path: str
    :param out_metadata_path: path to write the JSON file containing information on agent outputs.
    ...
    try:
        ...
        logger.progress("Agent successfully executed; see " + out_metadata_path)
        return result

    except Exception as error:
        errstr = "Agent wasn't successfully executed. ERROR: {}".format(error)
        logger.error(errstr)
        raise Exception(errstr)


if __name__ == "__main__":
    PARSER = argparse.ArgumentParser(description="VRE WfExS Agent")
